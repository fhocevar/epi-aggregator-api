...
class IndicatorsWeekly(Base):
    ...
    alert_level: Mapped[int] = mapped_column(Integer, nullable=True)
    raw: Mapped[dict] = mapped_column(JSONB, nullable=False)
    __table_args__ = (
        UniqueConstraint("source_code", "disease", "geo_code", "year", "epiweek",
...
class NotificationTarget(Base):
    ...
    enabled: Mapped[bool] = mapped_column(Boolean, default=True)
    disease_filter: Mapped[dict] = mapped_column(JSONB, nullable=True)
    geo_filter: Mapped[dict] = mapped_column(JSONB, nullable=True)
